...
dracoLoader.setDecoderPath("/40/draco/");
...
// load models
gltfLoader.load("/40/models.glb", (gltf) => {
  console.log(gltf);
  particles = {};
  particles.index = 0;
  ...
  for (const position of positions) {
    ...
    for (let i = 0; i < particles.maxCount; i++) {
      if (i < position.count) {
        ...
        newArray[i * 3] = originalArray[randomIndex * 3];
        newArray[i * 3 + 1] = originalArray[randomIndex * 3 + 1];
        newArray[i * 3 + 2] = originalArray[randomIndex * 3 + 2];
      }
    }
  ...
  // disable frustum culling to prevent particles from disappearing when they are outside of the camera frustum
  ...
  // debug
  gui.add(particles, "morph0").name("morph to model 0");
  gui.add(particles, "morph1").name("morph to model 1");
  gui.add(particles, "morph2").name("morph to model 2");
  gui.add(particles, "morph3").name("morph to model 3");
